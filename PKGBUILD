...
#	curl "${source}" -OLJ | tar -xz
	...
	ln -sf "libdrm_amdgpo.so.1.0.0" "libdrm_amdgpo.so.1"

	cp -r "${srcdir}/opencl/etc" "${pkgdir}/"
	mkdir -p ${pkgdir}/usr/lib
	sudo mv "${srcdir}/opencl/${shared}/libamdocl64.so" "${pkgdir}/usr/lib/"
	sudo mv "${srcdir}/opencl/${shared}/libamdocl12cl64.so" "${pkgdir}/usr/lib/"
	sudo mv "${srcdir}/libdrm/${shared/amdgpu-pro/amdgpu}/libdrm_amdgpo.so.1.0.0" "${pkgdir}/usr/lib/"
	sudo mv "${srcdir}/libdrm/${shared/amdgpu-pro/amdgpu}/libdrm_amdgpo.so.1" "${pkgdir}/usr/lib/"

        sudo mkdir -p "${pkgdir}/opt/amdgpu/share/libdrm"
        cd "${pkgdir}/opt/amdgpu/share/libdrm"
        sudo ln -sf /usr/share/libdrm/amdgpu.ids amdgpu.ids
